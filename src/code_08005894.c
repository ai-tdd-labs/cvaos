 ...
 *
 * Reads gEwramData->inputData.repeatedInput and updates the 8-character
 * name buffer at gEwramData+0x20 plus the glyph cursor stored in the
 * entity (unk_4F9 = selected glyph 0..0x23 on a 9-wide grid, unk_4FB =
 * character slot). A (bit 0) types the selected glyph (stored as index+2)
 * or, on the bottom row, confirms (0x22) / backspaces (0x23); B (bit 1)
 * backspaces; Start (bit 3) confirms immediately; L/R (0x300) move the
 * slot cursor; left/right (0x30) wrap within a grid row via Div/Mod;
 * up/down (0xC0) wrap by whole rows via Mod.
 *
 * On confirm the (possibly defaulted from sUnk_084F0D84) name is copied
 * to gEwramData->unk_60 offset 0x94 and, when unk_60.unk_60 bit 0 is set
 * for character 0, compared against the cheat names sUnk_080E0DCC
 * ("JULIUS" -> currentCharacter = 1, jingle 0x1E5), sUnk_080E0DDC
 * ("NOSOUL" -> unk_33C[2] |= 0x200000, jingle 0x199) and sUnk_080E0DD4
 * ("NOUSE" -> unk_33C[2] |= 0x100000, jingle 0x1B5); otherwise 0xF4.
 * Returns 1 once a name is confirmed, -1 when backspacing an empty name,
 * 0 otherwise; plays 0xF1 whenever the glyph cursor moved.
 *
 * Matching notes (agbcc/GCC 2.9): the input-mask arms must consume
 * temp_r4 via `temp_r4 &= mask` (a fresh `x & mask` temp gets coalesced
 * into r4 and forces a backup copy of temp_r4 into r1 at function entry);
 * the JULIUS/NOSOUL/NOUSE compare loops 2 and 3 must be goto-built so the
 * loop optimizer neither rotates them nor hoists the table addresses; the
 * name copy must go through a fresh single-use u8* local so the store
 * address is computed as (gEwramData + 0x94) + i instead of being
 * reassociated to (gEwramData + i) + 0x94.
 */
s32 sub_080083C8(struct EwramData_EntityData *param_0)
{
    ...
    else if (temp_r4 & 0x300)
    {
        ...
        temp_r4 &= 0x100;
        param_0->unk_4F8.unk_4F8_8.unk_4FB = temp_r4 ? (temp_r1 + 1) : (temp_r1 - 1);
    }
    ...
    switch (var_r8)
    {
        case 1:
            ...
            {
                var_1 = (u8 *)&gEwramData->unk_60.unk_94;
                var_1[var_r2_7] = temp_r5->unk_20[var_r2_7];
            }
